...
namespace QX.BackTesting.Strategies
{
    public partial class VoxLadder
    {
        ...
        const double BuyPremiumRatio = 0.30;

        const int MinSpreadWidth = 100;
        const int SpreadWidthBuyAdjBuffer = 50;

        // Phase 5: Moved to LadderTradeParameters.cs
        ...
        const double TargetPOP = 90;

        const double MinSellPremium = 20.0;
        const double MinBuyPremium = 5.0;
        const int MaxSpreadWidth = 1000;
        ...
        const int MaxConcurrentSpreads = 3;
        const bool ForceCloseOldestOnOverflow = false; // true = force-close oldest, false = skip entry
        ...
        const double StopLossFactor = 0.75;
        const double StopLossFactorPartialProfit = 0.00;
        const double StopLossFactorPartialLossBase = 1.25;
        const double SwingExitPct = 1.25;
        private readonly bool EnableTargetMaxLossAdj = true;
        private readonly bool OptunaMode = Environment.GetEnvironmentVariable("OPTUNA_MODE") == "1";

        #endregion


        #region Partial Booking / Far Leg (Phase 5: Moved to LadderTradeParameters.cs)
